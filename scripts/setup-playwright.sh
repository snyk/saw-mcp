...
#
# Works from a cloned repo or standalone — no repo checkout required.

set -euo pipefail

PLAYWRIGHT_CLI_PACKAGE="${PLAYWRIGHT_CLI_PACKAGE:-@playwright/cli@latest}"
...
echo "Installing ${PLAYWRIGHT_CLI_PACKAGE} globally..."
npm install -g "${PLAYWRIGHT_CLI_PACKAGE}"

# ── Verify binary is available ────────────────────────────────────────────────
if ! command -v playwright-cli &>/dev/null; then
  GLOBAL_BIN="$(npm prefix -g)/bin"
  echo "Error: playwright-cli binary not found after install." >&2
  echo "Make sure npm global bin is on your PATH: ${GLOBAL_BIN}" >&2
  exit 1
fi
